...
function AllUsersContainer(props) {
  ...
  const userClicked = (evt) => {
    ...
    const filteredUser = allUsers.find((idNum) => {
      return idNum.id === selectedUserId;
    });
    dispatch(actionCreator.interactingUser(filteredUser));
    userConversations.map((c) => {
      if (
        c.attributes.sender_id === selectedUserId ||
        c.attributes.recipient_id === selectedUserId
      ) {
  ...
  return (
    <>
      <Card.Group itemsPerRow={5}>
        {allUsers
          .filter((u) => u.id !== parseInt(localStorage.loggedInUserId))
          .map((auser) => {
            return (
